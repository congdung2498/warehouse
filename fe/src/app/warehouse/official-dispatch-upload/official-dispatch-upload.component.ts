...
import { OfficialDispatchImport } from '../Entity/OfficialDispatchImport';
import { Constants } from '../../shared/containts';

@Component({
  selector: 'app-official-dispatch-upload',
  templateUrl: './official-dispatch-upload.component.html',
  styleUrls: ['./official-dispatch-upload.component.css']
})
export class OfficialDispatchUploadComponent implements OnInit {
  ...
  cols = [
    ...
    { field: 'type', header: 'Loại công văn', width: '170px' },
    { field: 'officialDispatchName', header: 'Tên công văn', width: '170px' },
  ...
  fullResultCols = [
    { field: '', header: 'STT', width: '50px' },
    { field: 'type', header: 'Loại công văn', width: '200px' },
  ...
  ngOnInit() {
    this.title.setTitle('Tải tài liệu công văn');
  }
  ...
  peelOffData(excelDatas): OfficialDispatchImport[] {
    const peelOffDatas: OfficialDispatchImport[] = [];
      ...
      excelDatas.forEach(excel => {
        ...
        if (excel.cong_van_di && excel.cong_van_di.trim()) {
          peelOffDatas.push({
            ...
            levelBaomatOfficialDispatchDoc: excel.level_baomat_cong_van_di ? excel.level_baomat_cong_van_di.trim() : null
          });
        }
        ...
        if (excel.cong_van_den && excel.cong_van_den.trim()) {
          peelOffDatas.push({
            ...
            levelBaomatOfficialDispatchDoc: excel.level_baomat_cong_van_den ? excel.level_baomat_cong_van_den.trim() : null
          });
        }
      });
    }
  ...
  validateDataType(excelData: any) {
    ...
    if (firstRowKeys[1] !== 'cong_van_di') {
      return 1;
    }
    if (firstRowKeys[2] !== 'tai_lieu_cong_van_di') {
      return 1;
    }
    if (firstRowKeys[3] !== 'level_baomat_cong_van_di') {
      return 1;
    }
    if (firstRowKeys[4] !== 'cong_van_den') {
      return 1;
    }
    if (firstRowKeys[5] !== 'tai_lieu_cong_van_den') {
      return 1;
    }
    if (firstRowKeys[6] !== 'level_baomat_cong_van_den') {
      return 1;
    }
      ...
      if (
        Object.keys(ob).filter(keyObj => {
          return (
            ...
            keyObj !== 'level_baomat_cong_van_di' &&
            keyObj !== 'cong_van_den' &&
            keyObj !== 'tai_lieu_cong_van_den' &&
            keyObj !== 'level_baomat_cong_van_den' &&
  ...
  uploadToApi() {
    ...
    // push to api
    this.warehouseService.importOfficialDispatchData(uploadData).subscribe(res => {
  ...
  exportResultToExcel() {
    ...
    this.excelHelper.getOfficialDispatchResultExcel(data).subscribe(
      (res) => {
  ...
  downloadFullSample() {
    this.excelHelper.getOfficialDispatchTemplate().subscribe(
      (res) => {
        const fileName = 'File_mau_tai_du_lieu_cong_van.xlsx';
        const file: Blob = new Blob([res.body], { type: 'application/xlsx' });
        saveAs(file, fileName);
      }
    );
