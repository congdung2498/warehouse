...
import { VoucherDocImport } from '../Entity/VoucherDocImport';
...
@Component({
  selector: 'app-folder-voucher-upload',
  templateUrl: './folder-voucher-upload.component.html',
  styleUrls: ['./folder-voucher-upload.component.css']
})
export class FolderVoucherUploadComponent implements OnInit {
  ...
  cols = [
    ...
    { field: 'folderQrCode', header: 'Mã buộc HS', width: '150px' },
    { field: 'folderName', header: 'Tên buộc HS', width: '150px' },
    { field: 'createUser', header: 'Người tạo', width: '150px' },
    { field: 'type', header: 'Loại chứng từ', width: '150px' },
    { field: 'name', header: 'Tên chứng từ', width: '150px' },
    { field: 'docName', header: 'Tên tài liệu', width: '200px' },
  ...
  fullResultCols = [
    ...
    { field: 'folderQrCode', header: 'Mã buộc HS', width: '200px' },
    { field: 'folderName', header: 'Tên buộc HS', width: '350px' },
    { field: 'createUser', header: 'Người tạo', width: '180px' },
    { field: 'type', header: 'Loại chứng từ', width: '180px' },
    { field: 'name', header: 'Tên chứng từ', width: '350px' },
    { field: 'docName', header: 'Tên tài liệu', width: '350px' },
    { field: 'levelBaomatDoc', header: 'Mức chia sẻ tài liệu chứng từ', width: '240px' },
    { field: 'rowReport', header: 'Kết quả tải lên', width: '350px' },
  ];

  excelTemplate = ['stt', 'ma_thung', 'nguoi_tao', 'ma_hs', 'ten_hs', 'chung_tu', 'tai_lieu_chung_tu', 'level_baomat_chung_tu',
    'chung_ghi_so', 'tai_lieu_chung_tu_ghi_so', 'level_baomat_chung_tu_ghi_so', 'bang_tong_hop_thanh_toan',
    'tai_lieu_bang_tong_hop_thanh_toan', 'level_baomat_bang_tong_hop_thanh_toan'];
  ...
  ngOnInit() {
    this.title.setTitle('Tải tài liệu vào buộc hồ sơ chứng từ');
  }
  ...
  peelOffData(excelDatas): FolderVoucherImport[] {
    ...
    if (excelDatas && excelDatas.length > 0) {
      excelDatas.forEach(excel => {
        ...
        if (excel.chung_tu && excel.chung_tu.trim()) {
          peelOffDatas.push({
            tinBoxQrCode: excel.Ma_Thung,
            createUser: excel.Nguoi_Tao,
            folderQrCode: excel.Ma_HS,
            folderName: excel.Ten_HS,
            type: Constants.IMPORT_TYPE.VOUCHER,
            name: excel.chung_tu.trim(),
            docName: excel.tai_lieu_chung_tu ? excel.tai_lieu_chung_tu.trim() : null,
            levelBaomatDoc: excel.level_baomat_chung_tu ? excel.level_baomat_chung_tu.trim() : null
          });
        }
  ...
  uploadToApi() {
    ...
    // push to api
    this.warehouseService.importFolderVoucherData(uploadData).subscribe(res => {
  ...
  exportResultToExcel() {
    ...
    this.excelHelper.getFolderVoucherResultExcel(data).subscribe(
      (res) => {
  ...
  downloadFullSample() {
    this.excelHelper.getFolderVoucherTemplate().subscribe(
      (res) => {
        const fileName = 'File_mau_tai_du_buoc_ho_so_chung_tu.xlsx';
        const file: Blob = new Blob([res.body], { type: 'application/xlsx' });
        saveAs(file, fileName);
      }
    );
